<template>
    <div>
        <i class="fas fa-file-invoice fa-lg icon-tick-delivery" v-if="rowInfo.quantityPicked >= rowInfo.itemsToPick" v-b-tooltip.hover title="Create Invoice" @click.stop="getDeliveredPlants"></i>
    </div>
</template>

<script>
import jsPDF from 'jspdf'
import 'jspdf-autotable';
import moment from 'moment';
  export default {
    props: ['rowInfo'],
    data() {
      return{
        plantsToDeliver: [],
        quotePlants: [],
        customerRef: "",
        customerTel: "",
        comment: "",
        totalPrice: 0,
        siteRef: "",
        check1: false,
        check2: false,
        VAT: null,
      }
    },
    methods: {
      getDeliveredPlants() {
        if(this.check1 == false && this.check2 == false)
        {
          this.pickList();
          this.quote();
        } else if(this.check1 && this.check2) {
          this.check1 = false;
          this.check2 = false;
          this.getPlantPrices();
          //this.createInvoicePDF();
        }
        
      },
      pickList() {
        //Get Plants on the picklist
        this.axios.get('https://ahillsquoteservice.azurewebsites.net/api/picklist/detail?id=' + this.rowInfo.pickListId)
        .then((response) => {
          this.customerRef = response.data.CustomerRef;
          this.customerTel = response.data.CustomerTel;
          this.comment = response.data.Comment;
          this.changeData(response.data.PickListPlants);
        })
        .catch((error) => {
            alert("Error getting current picklists: " + error);
        });
      },
      quote() {
        //Get the plants on the quote to get the prices/total price
        this.axios.get('https://ahillsquoteservice.azurewebsites.net/api/quote/detail?id=' + this.rowInfo.quoteId)
        .then((response) => {
          this.totalPrice = (parseFloat(response.data.TotalPrice) / 100);
          this.siteRef = response.data.SiteRef;
          this.changeQuoteData(response.data.QuoteDetails);
         })
        .catch((error) => {
            alert("Error getting current picklists: " + error);
        });
      },
      changeData(items) {
        items.forEach(item => {
          this.plantsToDeliver.push({
            "plantName": item.PlantName + " " + item.FormSize,
            "quantity": item.QuantityToPick,
            "comment": item.SubbedFor,
            "pickedPlants": item.QuantityPicked,
            "outstandingPlants": item.QuantityToPick - item.QuantityPicked
          });
        });
        this.check1 = true;
        this.getDeliveredPlants()
      },
      changeQuoteData(items) {
        items.forEach(item => {
          this.quotePlants.push({
            "quotePlantName": item.PlantName + " " + item.FormSize,
            "quantity": item.Quantity,
            "comment": item.Comment,
            "price": (parseFloat(item.Price)/100),
          });
        });
        this.check2 = true;
        this.getDeliveredPlants()
      },
      getPlantPrices() {
        this.plantsToDeliver.forEach(element => {
          var currentFilteredPlant = this.quotePlants.filter(x => x.quotePlantName == element.plantName);
          element["price"] = currentFilteredPlant[0].price;
          element["rowTotal"] = (currentFilteredPlant[0].price * element.quantity).toFixed(2);
          element["rowVat"] = (((currentFilteredPlant[0].price * element.quantity)/100)*20).toFixed(2); //---------------------------Change this as well 
          if(element.comment != null)
          {
            element.comment = element.comment + " " + currentFilteredPlant[0].comment;
          } else {
            element.comment = currentFilteredPlant[0].comment;
          }
        });
        this.createInvoicePDF();
      },
      formatPriceForPDF() {
      //This is done after the quote has been saved to the database so changes will only show on created PDF
      //Map through the list of plants changing the price from "250p to 2.50"
      this.plantsToDeliver.map(o => Object.assign(o, {Price: "£"+(o.price/100).toFixed(2)}))
    },
      createInvoicePDF() {
        this.formatPriceForPDF();
        let pdfName = 'Invoice' + this.rowInfo.quoteId + this.rowInfo.pickListId;
        var doc = new jsPDF('p', 'pt');
        var columns = [
          {title: "Quantity", dataKey: "quantity"},
          {title: "Plant", dataKey: "plantName"},
          {title: "Comment", dataKey: "comment"}, //This can be a subsitute too, although ill need to bring comment in too?
          {title: "Price", dataKey: "price"},
          {title: "Total exVAT", dataKey: "rowTotal"},
          {title: "VAT", dataKey: "rowVat"},
        ];
        var companyLogo = {
          src: 'data:image/png;base64,'
        }
        var companyInfo = "Station Nurserys\n" +
                          "Stokesley\n" +
                          "Middlesbrough\n" +
                          "TS9 7AB\n" +
                          "Tel: 01642 711281\n" +
                          "Email: enquiries@hillandsons.co.uk\n" +
                          "Web: hillandsons.co.uk\n" +
                          "Stock: hillsstock.co.uk"
        var date = moment(new Date).format('DD/MM/YYYY');
        var noteInfo = "Quote Ref: " + this.rowInfo.quoteId + "\n" +
                       "PickList Ref: " + this.rowInfo.pickListId + "\n" +
                       "Customer Ref: " + this.customerRef + "\n" +
                       "Customer Name: " + this.rowInfo.customerName + "\n" +
                       "Customer Tel: " + this.customerTel + "\n" +
                       "Invoice Date: " + date + "\n" +
                       "Number of items: " + this.rowInfo.quantityPicked

        doc.addImage(companyLogo.src, 'PNG', 30, 30, 100, 75);
			  doc.setFontSize(20);
			  doc.text("Invoice", 445, 25)
        doc.setFontSize(10);
        doc.text(companyInfo, 135, 35);
        doc.text(noteInfo, 350, 35);
        //Table Stuff
        doc.autoTable(columns, this.plantsToDeliver, {theme: 'striped', startY: 130, startX: 30,
                                                  styles: {
                                                      overflow: 'linebreak',
                                                      },
                                                  columnStyles: {
                                                    0: {cellWidth: 50},
                                                    1: {cellWidth: 100},
                                                    2: {cellWidth: 100},
                                                    3: {cellWidth: 50},
                                                    4: {cellWidth: 50},
                                                    5: {cellWidth: 50},
                                                    }    
                                                  });
        let finalY = doc.autoTable.previous.finalY;
        let VAT = 20; //-------------------------------------------------------------------Remove this shit eventually and just tag onto service and grab VAT on load
        let quotePrice = (this.totalPrice).toFixed(2);
			  let quoteVAT = (quotePrice/100*VAT).toFixed(2);
        let priceAfterVAT = (parseFloat(quotePrice)+parseFloat(quoteVAT)).toFixed(2);

        doc.setFontSize(12);
        doc.text("Total Exc. VAT: £" + quotePrice, 380, finalY+20);
        doc.text("VAT:                  £" + quoteVAT, 380, finalY+35);
        doc.text("Total Inc. VAT:  £" + priceAfterVAT, 380, finalY+50);

        doc.setFontSize(10);
        doc.text("Thank you for your order",120,finalY+20)
        doc.setFontSize(8);
        doc.text("We look forward to supplying you again in the future", 90, finalY+30);
        doc.text("Vat No 258 1678 29 : Plant Passport No GB/12803 : Company No 4395902", 60, finalY+40);
        doc.text("Directors D.Hill : M.Hill : J.Hill : S.Hill", 120, finalY+50);

        doc.save(pdfName + '.pdf');
      },
    }
  }
</script>

<style scoped>
 .icon-tick-delivery {
    color: rebeccapurple;
    margin-left: 2px;
  }
</style>